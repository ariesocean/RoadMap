...
- **Code Migration**: Login UI code SHALL be migrated from `roadmap-manager-login/src/App.tsx` instead of redesigning
- **State Reuse**: Login/Logout SHALL reuse existing `isConnected` state from `useTaskStore` - login = Connected, logout = Disconnected
- Use localStorage for persisting username
- No backend API integration - frontend UI only
- Password validation is simulated - no actual verification
- **Security Note**: This is a frontend-only prototype with no real authentication security
