...
**Refactoring Note**: Internal storage implementation now uses centralized `storage.ts` utility for localStorage operations.
...
**Refactoring Note**: ID generation now uses centralized `idGenerator.ts` utility.
...
**Refactoring Note**: Session sorting logic now uses centralized `sessionUtils.ts` utility.
...
- **WHEN** server returns 401 or 403
...
**Refactoring Note**: Uses centralized `storage.ts` for cleanup operations.
