...
async function registerUser(username, _email, _password, deviceId) {
    ensureUsersDir();
...
async function loginUser(username, _password, deviceId, deviceInfo) {
    ensureUsersDir();
    const existingUsers = fs.readdirSync(USERS_DIR).filter(f => f !== 'ports.json');
    let userId = null;
    for (const dir of existingUsers) {
    ...
    let device = devices.devices.find((d) => d.deviceId === deviceId);
    if (!device) {
        ...
        device = devices.devices.find((d) => d.deviceId === deviceId);
    }
    else {
...
async function autoLogin(deviceId) {
    ensureUsersDir();
    const existingUsers = fs.readdirSync(USERS_DIR).filter(f => f !== 'ports.json');
...
async function removeDevice(userId, deviceId) {
    ...
    devices.devices = devices.devices.filter((d) => d.deviceId !== deviceId);
    fs.writeFileSync(devicesPath, JSON.stringify(devices, null, 2));
}
...
async function startOpenCodeServer(port) {
    return new Promise((resolve, reject) => {
        ...
        const proc = (0, child_process_1.spawn)('opencode', ['serve', '--port', String(port)], {
            cwd: PROJECT_DIR,
            env,
            detached: true,
            stdio: 'ignore'
...
async function startUserOpenCodeServer(userId) {
...
function killOpenCodeProcess(port) {
    try {
        const output = (0, child_process_1.execSync)(`pgrep -f "opencode.*serve.*--port ${port}"`, { encoding: 'utf-8' });
        if (output.trim()) {
            (0, child_process_1.execSync)(`kill ${output.trim()}`);
...
async function stopUserOpenCodeServer(userId) {
    const port = getUserPort(userId);
    if (!port)
        return;
    if (!await checkPort(port)) {
...
app.get('/session', async (req, res) => {
    ...
    try {
        ...
        const roadmapSessions = sessions.filter((s) => s.directory === getCurrentUserDir() &&
            !s.parentID &&
            !/\(@.*subagent\)/i.test(s.title || '') &&
            !(s.title || '').startsWith('modal-prompt:'));
        roadmapSessions.sort((a, b) => (b.time?.created || 0) - (a.time?.created || 0));
